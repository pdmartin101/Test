...
{
    internal class PdmConsoleLogger : ILogger
    {
        ...
        private readonly PdmConsoleLoggerProcessor _queueProcessor;

        [ThreadStatic]
        private static StringBuilder _logBuilder;

        static PdmConsoleLogger()
        {
        ...
        internal PdmConsoleLogger(string name, PdmConsoleLoggerProcessor loggerProcessor)
        {
            if (name == null)
            {
        ...
        internal PdmConsoleLoggerOptions Options { get; set; }
...
//            Debug.Assert(format >= PdmConsoleLoggerFormat.Default && format <= PdmConsoleLoggerFormat.Systemd);
            ...
            if (format == PdmConsoleLoggerFormat.Default)
            {
                entry = CreateDefaultLogMessage(logBuilder, logLevel, logName, eventId, message, exception);
            }
            else if (format == PdmConsoleLoggerFormat.Systemd)
            {
